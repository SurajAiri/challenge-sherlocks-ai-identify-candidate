...
def test_llm_name_role_emits_for_candidate_on_positive_verdict(monkeypatch):
    async def fake_structured_completion(
        *, system_prompt, user_prompt, schema, use_cache=True
    ):
        assert schema is NameRoleVerdict
        return NameRoleVerdict(
            verdict="candidate", confidence=0.8, reasoning="Nickname of the candidate."
        )

    monkeypatch.setattr(
        "engine.identifiers.llm_name_role.structured_completion",
    ...
    frames = [
        _context(),
        _event(0, SimEventType.PARTICIPANT_JOIN, "p_a", display_name="S. Thapa"),
    ]
    ...
    assert (
        "p_a" in final["evidence"] or probs["p_a"] > 0
    )  # some positive signal recorded
...
def test_llm_name_role_emits_against_candidate_on_interviewer_verdict(monkeypatch):
    async def fake_structured_completion(
        *, system_prompt, user_prompt, schema, use_cache=True
        ...
        return NameRoleVerdict(
            verdict="interviewer",
            confidence=0.9,
            reasoning="Matches known interviewer.",
        )
    ...
    frames = [
        _context(),
        _event(0, SimEventType.PARTICIPANT_JOIN, "p_b", display_name="Alex R."),
    ]
    messages = asyncio.run(_drive(frames, registry))
...
def test_llm_name_role_emits_nothing_on_unclear_verdict(monkeypatch):
    async def fake_structured_completion(
        *, system_prompt, user_prompt, schema, use_cache=True
        ...
        return NameRoleVerdict(
            verdict="unclear", confidence=0.4, reasoning="Could be anyone."
        )
    ...
    frames = [
        _context(),
        _event(0, SimEventType.PARTICIPANT_JOIN, "p_c", display_name="MacBook Pro"),
    ]
    messages = asyncio.run(_drive(frames, registry))
...
def test_llm_name_role_fails_open_when_llm_call_raises(monkeypatch):
    ...
    frames = [
        _context(),
        _event(0, SimEventType.PARTICIPANT_JOIN, "p_d", display_name="Whoever"),
    ]
    messages = asyncio.run(_drive(frames, registry))
...
def test_llm_transcript_role_emits_evidence_from_batched_assessment(monkeypatch):
    async def fake_structured_completion(
        *, system_prompt, user_prompt, schema, use_cache=True
        ...
        return TranscriptRoleAssessment(
            assessments=[
                ParticipantRoleVerdict(
                    participant_id="p_cand",
                    verdict="interviewee",
                    confidence=0.85,
                    reasoning="Answers at length.",
                ),
                ParticipantRoleVerdict(
                    participant_id="p_int",
                    verdict="interviewer",
                    confidence=0.9,
    ...
    monkeypatch.setattr(
        "engine.identifiers.llm_transcript_role.structured_completion",
        fake_structured_completion,
    )
